...
# Generate en_US.UTF-8 in the image: live-build 2025 dropped native locale
# handling (no LB_LOCALES, no locale-gen anywhere), while LB_BOOTAPPEND_LIVE
# promises locales=en_US.UTF-8. Needs the locales package (ygg.list.chroot).
tee config/hooks/normal/1030-generate-locales.hook.chroot <<'YGGEOF'
#!/bin/sh
...
if command -v locale-gen >/dev/null 2>&1; then
    ...
    grep -qE '^en_US\.UTF-8 UTF-8$' /etc/locale.gen || echo 'en_US.UTF-8 UTF-8' >>/etc/locale.gen
    locale-gen >/dev/null
fi
YGGEOF
...
# Locale generation set (1030-generate-locales.hook.chroot runs locale-gen)
locales
...
# Disk health monitoring + alert delivery.
# All three are required for a smartd alert to actually leave the machine:
# smartmontools raises it, msmtp-mta provides /usr/sbin/sendmail, and
# bsd-mailx provides /usr/bin/mail -- which is what Debian's
# /etc/smartmontools/run.d/10mail actually invokes. Drop bsd-mailx and smartd
# fails with "does not have /usr/bin/mail" and mails nothing.
smartmontools
msmtp
msmtp-mta
bsd-mailx
...
# Site pool list for ygg-import-zpool-at-boot (private values via YGG_EXTRA_POOLS)
mkdir -p "config/includes.chroot/etc/default"
cat > "config/includes.chroot/etc/default/ygg-import-zpool" <<YGGPOOL
EXTRA_POOLS="${YGG_EXTRA_POOLS:-}"
YGGPOOL
...
MACVLAN_CIDR="${YGG_MACVLAN_CIDR:-198.51.100.250/24}"
MACVLAN_ROUTE="${YGG_MACVLAN_ROUTE:-198.51.100.0/24}"
...
# --- Disk Health Alerts + Jewel Backup ---
ALERT_EMAIL="${YGG_ALERT_EMAIL:-}"
SMTP_RELAY_HOST="${YGG_SMTP_RELAY_HOST:-}"
SMTP_RELAY_PORT="${YGG_SMTP_RELAY_PORT:-25}"
MAIL_FROM="${YGG_MAIL_FROM:-root@yggdrasil.example}"
MAIL_EHLO_DOMAIN="${YGG_MAIL_EHLO_DOMAIN:-yggdrasil.example}"
JEWEL_ENABLE="${YGG_JEWEL_BACKUP_ENABLE:-false}"
JEWEL_DEST="${YGG_JEWEL_BACKUP_DEST:-}"
JEWEL_DATASETS="${YGG_JEWEL_BACKUP_DATASETS:-}"
JEWEL_STRIP_PREFIX="${YGG_JEWEL_BACKUP_SRC_STRIP_PREFIX:-}"
JEWEL_ONCALENDAR="${YGG_JEWEL_BACKUP_ONCALENDAR:-*-*-* 04:30:00 UTC}"
JEWEL_KEEP_SRC="${YGG_JEWEL_BACKUP_KEEP_SRC:-3}"
JEWEL_KEEP_DST="${YGG_JEWEL_BACKUP_KEEP_DST:-30}"

# Pass 1 writes the build-time values. Pass 2 appends the body inside a QUOTED
# heredoc so the embedded scripts keep their own $variables verbatim.
tee config/hooks/normal/9110-storage-alerts.hook.chroot <<EOF
#!/bin/bash
...
JEWEL_ENABLE="${JEWEL_ENABLE}"
JEWEL_DEST="${JEWEL_DEST}"
JEWEL_DATASETS="${JEWEL_DATASETS}"
JEWEL_STRIP_PREFIX="${JEWEL_STRIP_PREFIX}"
JEWEL_ONCALENDAR="${JEWEL_ONCALENDAR}"
JEWEL_KEEP_SRC="${JEWEL_KEEP_SRC}"
JEWEL_KEEP_DST="${JEWEL_KEEP_DST}"
EOF

cat >> config/hooks/normal/9110-storage-alerts.hook.chroot <<'EOF'
...
EOF
chmod +777 config/hooks/normal/9110-storage-alerts.hook.chroot
...
tee config/hooks/normal/9106-boot-telemetry.hook.chroot <<'EOF'
#!/bin/bash
...
EOF
chmod +777 config/hooks/normal/9106-boot-telemetry.hook.chroot
...
# Yggclient apps: latest yggterm release (github) + ynpm user apps.
echo '' | base64 -d > config/hooks/normal/9202-yggclient-apps.hook.chroot
chmod +755 config/hooks/normal/9202-yggclient-apps.hook.chroot
...
# ---------------------------------------------------------------------------
# Certificate wire sentinel (ygg-cert-sentinel, baked 2026-09-11). Probes the
# TLS certificate each endpoint actually SERVES (not the file on disk) daily
# at 07:20 and fails the unit under CERT_SENTINEL_MIN_DAYS remaining, so a
# missed reload surfaces in `systemctl --failed` instead of at expiry.
#
# Site endpoints come from ygg.local.toml:
#   cert_sentinel_endpoints = "host[:port][:starttls] ..."   → YGG_CERT_SENTINEL_ENDPOINTS
# and are baked into /etc/ygg/cert-sentinel.env at image build time (they
# must not enter the GIT repo; the built image is site-specific anyway).
# Without the key the monitor ships installed but DISABLED — the sentinel
# script itself refuses to exit 0 with an empty endpoint list.
# ---------------------------------------------------------------------------
{
  echo '#!/bin/sh'
  echo 'set -e'
  echo 'mkdir -p /etc/ygg /usr/local/sbin'
  echo "cat > /usr/local/sbin/cert-sentinel.sh <<'SCRIPTBODYEOF'"
  cat scripts/assets/cert-sentinel.sh
  echo 'SCRIPTBODYEOF'
  echo 'chmod 0755 /usr/local/sbin/cert-sentinel.sh'
  echo "cat > /etc/systemd/system/ygg-cert-sentinel.service <<'UNITBODYEOF'"
  cat scripts/assets/ygg-cert-sentinel.service
  echo 'UNITBODYEOF'
  echo "cat > /etc/systemd/system/ygg-cert-sentinel.timer <<'TIMERBODYEOF'"
  cat scripts/assets/ygg-cert-sentinel.timer
  echo 'TIMERBODYEOF'
  echo 'if [ -n "'"$YGG_CERT_SENTINEL_ENDPOINTS"'" ]; then'
  echo '  cat > /etc/ygg/cert-sentinel.env <<ENVEOF'
  echo 'CERT_SENTINEL_ENDPOINTS="'"$YGG_CERT_SENTINEL_ENDPOINTS"'"'
  echo 'CERT_SENTINEL_MIN_DAYS='"${YGG_CERT_SENTINEL_MIN_DAYS:-10}"
  echo 'ENVEOF'
  echo '  chmod 0644 /etc/ygg/cert-sentinel.env'
  echo '  systemctl enable ygg-cert-sentinel.timer 2>/dev/null || true'
  echo '  echo "cert-sentinel: enabled with site endpoints"'
  echo 'else'
  echo '  echo "cert-sentinel: YGG_CERT_SENTINEL_ENDPOINTS not set in site config - installed DISABLED"'
  echo 'fi'
} > config/hooks/normal/9120-cert-sentinel.hook.chroot
chmod 0755 config/hooks/normal/9120-cert-sentinel.hook.chroot
